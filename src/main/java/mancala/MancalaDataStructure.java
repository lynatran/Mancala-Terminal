/* Code provided by CIS*2430 Instructor, Dr. Judi McCuaig */
...
public class MancalaDataStructure implements Serializable {
    ...
    public String toString() {
        ...
        final Store store1 = (Store) data.get(storePos(1));
        final Store store2 = (Store) data.get(storePos(2));


        boardString.append("-------------------------------------------------");
        boardString.append("Top Row (P2 Pits):");
        boardString.append("-------------------------------------------------\n");
        boardString.append("\t\t");

        // Display player two's pits and store (top row)
        for (int i = 12; i > 6; i--) {
            boardString.append("Pit ").append(pitTop).append(":\t\t");
            pitTop--;
        }
        ...
        for (int i = 12; i > 6; i--) {
            boardString.append(data.get(i).getStoneCount() + "\t\t");
        }

        boardString.append("\n" + store2.getOwner().getName() + "'s Store\t\t\t\t\t\t\t\t\t\t\t\t");
        boardString.append(store1.getOwner().getName() + "'s Store");

        boardString.append("\n" + store2.getStoneCount() + "\t\t\t\t\t\t\t\t\t\t\t\t\t\t");
        boardString.append(store1.getStoneCount());
        boardString.append("\n\t\t");
    
        // Display player one's pits and store (bottom row)
        for (int i = 6; i > 0; i--) {
            boardString.append("Pit ").append(pitBottom).append(":\t\t");
            pitBottom++;
        }
        ...
        boardString.append("\n-----------------------------------------------");
        boardString.append("Bottom Row (P1 Pits):");
        boardString.append("-------------------------------------------------\n");
