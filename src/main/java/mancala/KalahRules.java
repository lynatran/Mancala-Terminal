...
public class KalahRules extends GameRules {
   ...
   public int captureStones(final int stoppingPoint) {
        ...
        if (stoppingPit == 1 && oppositePitNum > 0) {
            ...
            final int capturedStones = getDataStructure().removeStones(stoppingPoint) + getDataStructure().removeStones(oppositePitIndex);
    ...
    public int distributeStones(final int startingPoint) {
        ...
        getDataStructure().setIterator(startingPoint, getCurrentPlayer(), false);
    ...
    public int moveStones(final int startPit, final int playerNum) throws InvalidMoveException {
        ...
        if (playerNum == 1 && stopPit > 6 && (stopPit-1) != -1) {
            stopPit--;
        } else if (playerNum == 2 && stopPit < 7 && (stopPit-1) != -1) {
            stopPit--;
        }
        ...
        // If the last stone was placed in the player's store, give them a free turn
        final boolean lastStoneInStore = freeTurnCheck(playerNum, stopPit, addedStoreCount);
        if (lastStoneInStore) {
            return -1;
        }
    ...
    private boolean freeTurnCheck(final int playerNum, final int stopPit, final int addedStoreCount) {
        return playerNum == 1 && stopPit == 6 && addedStoreCount == 1
               || playerNum == 2 && stopPit == 0 && addedStoreCount == 1;
    }
}
